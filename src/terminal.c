...
static bool parseMouseSGR(const char *seq, int *Cb, int *Cx, int *Cy, char *fin)
{
  if (*seq != '<')
    return false;
  seq++;
...
EditorInput editorReadKey(void)
{
  ...
  if (c == ESC)
  {
    ...
    if (strcmp(seq, "[200~") == 0)
    {
      ...
      while (true)
      {
        ...
        if (c == ESC)
        {
          ...
          size_t index;
          for (index = 0; index < sizeof(end_seq) / sizeof(end_seq[0]); index++)
          {
    ...
    for (size_t i = 0; i < sizeof(sequence_lookup) / sizeof(sequence_lookup[0]); i++)
    {
...
{
  writeConsoleStr(MOUSE_DISABLE BRACKETED_PASTE_DISABLE SWAP_DISABLE ANSI_CLEAR ANSI_CURSOR_SHOW);
  disableRawMode();
}
