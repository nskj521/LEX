...
void *json_arena_realloc(JsonArena *arena, void *ptr, size_t old_size, size_t size);
...
static inline size_t json__arena_alignment_loss(size_t bytes_allocated, size_t alignment)
{
...
static void json__arena_alloc_new_block(JsonArena *arena, size_t requested_size)
{
  ...
  while ((c = *state->p++))
  {
    ...
    else if ((c >= '0' && c <= '9') || c == '-')
    {
      ...
      if (c == '-')
      {
        ...
        if (!(c >= '0' && c <= '9'))
        {
          return json__token_error(state, "No number after minus sign at position %ld",
                                   state->p - state->start);
        }
      }
      ...
      if (c == '.')
      {
        ...
        {
          return json__token_error(state, "Unterminated fractional number at position %ld",
                                   state->p - state->start);
        }
      }
      ...
      if (c == 'e' || c == 'E')
      {
        ...
        {
          return json__token_error(state, "Exponent part is missing a number at position %ld",
                                   state->p - state->start);
        }
      }
      state->p--;
    ...
    else if (c == '"')
    {
      ...
      while ((c = *state->p++) != '"')
      {
        if (c == '\0')
        {
          return json__token_error(state, "Unterminated string at position %ld",
                                   state->p - state->start);
        }
        ...
        if (c < 32 || c == 127)
        {
          return json__token_error(state,
        ...
        if (c == '\\')
        {
          ...
          switch (c)
          {
            ...
            // TODO: Add Unicode
            case 'u':
              return json__token_error(state, "Unicode escape not implemented");
            default:
              return json__token_error(state, "Bad escaped character at position %ld",
                                       state->p - state->start);
          }
        }
        ...
        if (size + 2 > capacity)
        {
          token.string = json_arena_realloc(state->arena, token.string, capacity, capacity * 2);
          capacity *= 2;
        }
    ...
    else
    {
      switch (c)
      {
        ...
        case ':':
          ...
          break;
        default:
          return json__token_error(state, "Unexpected token '%c' at position %ld", c,
                                   state->p - state->start);
      }
      return token;
    }
...
static JsonValue *json__parse_object(JsonParserState *state)
{
  ...
  while (token.type == JSON_TOKEN_STRING)
  {
    ...
    if (token.type == JSON_TOKEN_COMMA)
    {
      ...
      {
        return json__error(state, "Expected double-quoted property name");
      }
    }
    else
    {
      return json__error(state, "Expected ',' or '}' after property value");
    }
  }
...
static JsonValue *json__parse_array(JsonParserState *state)
{
  ...
  while (token.type != JSON_TOKEN_RBRACKET)
  {
    if (array->size > 0)
    {
      ...
      {
        return json__error(state, "Expected ',' or ']' after array element");
      }
    }
    ...
    if (array->size + 1 > capacity)
    {
      ...
      array->data = json_arena_realloc(state->arena, array->data, sizeof(JsonValue *) * capacity,
