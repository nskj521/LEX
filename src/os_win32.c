...
void enableRawMode(void)
{
  ...
  orig_in_mode = mode;
  mode |= ENABLE_WINDOW_INPUT | ENABLE_EXTENDED_FLAGS | ENABLE_VIRTUAL_TERMINAL_INPUT;
  mode &=
...
FileInfo getFileInfo(const char *path)
{
  ...
  HANDLE hFile = CreateFileW(w_path, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
...
const char *dirGetName(const DirIter *iter)
{
  ...
  WideCharToMultiByte(CP_UTF8, 0, iter->find_data.cFileName, -1, dir_name, EDITOR_PATH_MAX, NULL,
                      false);
  return dir_name;
}
...
char *getFullPath(const char *path)
{
  ...
  WideCharToMultiByte(CP_UTF8, 0, w_resolved_path, -1, resolved_path, EDITOR_PATH_MAX, NULL, false);
...
void argsInit(int *argc, char ***argv)
{
  ...
  *argv = malloc_s(*argc * sizeof(char *));
  ...
  {
    int size   = WideCharToMultiByte(CP_UTF8, 0, w_argv[i], -1, NULL, 0, NULL, NULL);
    (*argv)[i] = malloc_s(size);
    WideCharToMultiByte(CP_UTF8, 0, w_argv[i], -1, (*argv)[i], size, NULL, NULL);
  }
}
