...
// --- MODIFIKASI: Tambahkan variabel untuk menunggu tombol Enter setelah save ---
static bool waiting_for_enter_after_save = false;
// --- AKHIR MODIFIKASI ---
...
static void editorExplorerScrollToSelect(void)
{
  ...
  else if ((int) gEditor.explorer.selected_index >= gEditor.explorer.offset + gEditor.display_rows)
  {
    gEditor.explorer.offset = gEditor.explorer.selected_index - gEditor.display_rows + 1;
  }
...
static inline void editorExplorerShow(void)
{
  ...
  {
    gEditor.explorer.width = gEditor.explorer.prefered_width ? gEditor.explorer.prefered_width
...
static bool editorExplorerProcessKeypress(EditorInput input)
{
  if (input.type == CHAR_INPUT)
  {
    ...
    for (size_t i = 0; i < gEditor.explorer.flatten.size; i++)
    {
      ...
      if (tolower(getBaseName(gEditor.explorer.flatten.data[index]->filename)[0]) == c)
      {
  ...
  {
    case WHEEL_UP:
      if (getMousePosField(input.data.cursor.x, input.data.cursor.y) != FIELD_EXPLORER)
        return false;
      if (gEditor.explorer.offset > 0)
      {
        gEditor.explorer.offset =
            (gEditor.explorer.offset - 3) > 0 ? (gEditor.explorer.offset - 3) : 0;
      }
      break;
    ...
    case WHEEL_DOWN:
      if (getMousePosField(input.data.cursor.x, input.data.cursor.y) != FIELD_EXPLORER)
        return false;
      if ((int) gEditor.explorer.flatten.size - gEditor.explorer.offset > gEditor.display_rows)
      {
    ...
    case ARROW_DOWN:
      if (gEditor.explorer.selected_index + 1 >= (int) gEditor.explorer.flatten.size)
        break;
      gEditor.explorer.selected_index++;
      editorExplorerScrollToSelect();
      break;
    ...
    case PAGE_DOWN:
      if (gEditor.explorer.selected_index != gEditor.explorer.offset + gEditor.display_rows - 1)
      {
        gEditor.explorer.selected_index = gEditor.explorer.offset + gEditor.display_rows - 1;
      }
      else
      {
        gEditor.explorer.selected_index += gEditor.display_rows;
    ...
    case MOUSE_PRESSED:
      ...
      if (input.data.cursor.y > (int) gEditor.explorer.flatten.size - gEditor.explorer.offset)
        break;
      gEditor.explorer.selected_index = input.data.cursor.y - 1 + gEditor.explorer.offset;
      editorExplorerNodeClicked();
      break;

    // --- MODIFIKASI: Ubah shortcut keluar di mode explorer dari Ctrl+Q ke Ctrl+X ---
    case CTRL_KEY('x'):
...
void editorScrollToCursor(void)
{
  ...
  {
    rx = editorRowCxToRx(&gCurFile->row[gCurFile->cursor.y], gCurFile->cursor.x);
  }
...
int getMousePosField(int x, int y)
{
  ...
  if (y == gEditor.screen_rows - 2 && gEditor.state != EDIT_MODE && gEditor.state != EXPLORER_MODE)
    return FIELD_PROMPT;
...
void editorMoveCursor(int key)
{
  ...
  switch (key)
  {
    case ARROW_LEFT:
      ...
      {
        gCurFile->cursor.x =
            editorRowPreviousUTF8(&gCurFile->row[gCurFile->cursor.y], gCurFile->cursor.x);
        gCurFile->sx = editorRowCxToRx(&gCurFile->row[gCurFile->cursor.y], gCurFile->cursor.x);
      }
    ...
    case ARROW_RIGHT:
      ...
      {
        gCurFile->cursor.x =
            editorRowNextUTF8(&gCurFile->row[gCurFile->cursor.y], gCurFile->cursor.x);
        gCurFile->sx = editorRowCxToRx(&gCurFile->row[gCurFile->cursor.y], gCurFile->cursor.x);
      }
    ...
    case ARROW_UP:
      ...
      {
        gCurFile->cursor.y--;
        gCurFile->cursor.x = editorRowRxToCx(&gCurFile->row[gCurFile->cursor.y], gCurFile->sx);
      }
      break;
  ...
  row = (gCurFile->cursor.y >= gCurFile->num_rows) ? NULL : &gCurFile->row[gCurFile->cursor.y];
...
static int findNextCharIndex(const EditorRow *row, int index, IsCharFunc is_char)
{
...
static int findPrevCharIndex(const EditorRow *row, int index, IsCharFunc is_char)
{
...
static void editorMoveCursorWordLeft(void)
{
  ...
  const EditorRow *row = &gCurFile->row[gCurFile->cursor.y];
  gCurFile->cursor.x   = findPrevCharIndex(row, gCurFile->cursor.x, isIdentifierChar);
  gCurFile->cursor.x   = findPrevCharIndex(row, gCurFile->cursor.x, isNonIdentifierChar);
  gCurFile->sx         = editorRowCxToRx(&gCurFile->row[gCurFile->cursor.y], gCurFile->cursor.x);
}
...
static void editorMoveCursorWordRight(void)
{
  ...
  const EditorRow *row = &gCurFile->row[gCurFile->cursor.y];
  gCurFile->cursor.x   = findNextCharIndex(row, gCurFile->cursor.x, isIdentifierChar);
  gCurFile->cursor.x   = findNextCharIndex(row, gCurFile->cursor.x, isNonIdentifierChar);
  gCurFile->sx         = editorRowCxToRx(&gCurFile->row[gCurFile->cursor.y], gCurFile->cursor.x);
}
...
static void editorCloseFile(int index)
{
  ...
  {
    editorMsg("File has unsaved changes. Press again to close file "
              "anyway.");
    close_protect = index;
    return;
  }
  ...
  if (index < gEditor.file_index || (gEditor.file_index == index && index == gEditor.file_count))
  {
...
{
  // --- MODIFIKASI: Tambahkan logika untuk menunggu Enter setelah save ---
  if (waiting_for_enter_after_save)
  {
    EditorInput input = editorReadKey();
    if (input.type == '\r')
    {  // Jika Enter ditekan
      waiting_for_enter_after_save = false;
      editorMsgClear();
    }
    editorFreeInput(&input);
    return;  // Jangan proses input lainnya
  }
  // --- AKHIR MODIFIKASI ---
  ...
  switch (input.type)
  {
    ...
    // --- MODIFIKASI: Pindahkan fungsi buka file dari Ctrl+O ke Ctrl+S ---
    // Open file
    case CTRL_KEY('s'):
  ...
  switch (c)
  {
    ...
    // --- MODIFIKASI: Ubah shortcut keluar dari Ctrl+Q ke Ctrl+X ---
    // Quit editor
    case CTRL_KEY('x'):
    {
      close_protect = -1;
      ...
      {
        editorMsg("Files have unsaved changes. Press ^X again to quit "
                  "anyway.");
        quit_protect = false;
        return;
      }
    ...
    // --- MODIFIKASI: Hapus shortcut save lama (Ctrl+S) dan ganti dengan save baru (Ctrl+O) ---
    // Save
    case CTRL_KEY('o'):
      should_scroll = false;
    ...
    case SHIFT_HOME:
    {
      int start_x = findNextCharIndex(&gCurFile->row[gCurFile->cursor.y], 0, isNonSpace);
      if (start_x == gCurFile->cursor.x)
        start_x = 0;
      gCurFile->cursor.x             = start_x;
      gCurFile->sx                   = editorRowCxToRx(&gCurFile->row[gCurFile->cursor.y], start_x);
    ...
    case BACKSPACE:
    {
      ...
      bool should_delete_bracket =
          ...
          (isCloseBracket(gCurFile->row[gCurFile->cursor.y].data[gCurFile->cursor.x]) ==
      ...
      char deleted_char = '\0';
      if (gCurFile->cursor.x != 0)
        deleted_char = gCurFile->row[gCurFile->cursor.y].data[gCurFile->cursor.x - 1];
      editorMoveCursor(ARROW_LEFT);
      if (CONVAR_GETINT(backspace) && deleted_char == ' ')
      {
        ...
        {
          int idx = editorRowCxToRx(&gCurFile->row[gCurFile->cursor.y], gCurFile->cursor.x);
    ...
    // --- MODIFIKASI: Pindahkan fungsi cut dari Ctrl+X ke Alt+X ---
    // Action: Cut
    case ALT_KEY('x'):
    {
      if (gCurFile->num_rows == 1 && gCurFile->row[0].size == 0)
      ...
      if (!gCurFile->cursor.is_selected)
      {
        ...
        // Delete line
        EditorSelectRange range = {0, gCurFile->cursor.y, gCurFile->row[gCurFile->cursor.y].size,
                                   gCurFile->cursor.y};
    ...
    case CTRL_KEY('v'):
    {
      EditorClipboard *clipboard = (c == PASTE_INPUT) ? &input.data.paste : &gEditor.clipboard;
    ...
    {
      const EditorRow *row = &gCurFile->row[gCurFile->cursor.y];
      if (gCurFile->cursor.x < row->size && !isIdentifierChar(row->data[gCurFile->cursor.x]))
      {
        should_scroll = false;
        break;
      }
    ...
    case PAGE_DOWN:
    {
      gCurFile->cursor.is_selected   = (c == SHIFT_PAGE_UP || c == SHIFT_PAGE_DOWN);
      gCurFile->bracket_autocomplete = 0;
    ...
    case SHIFT_ALT_DOWN:
      ...
      editorInsertRow(gCurFile, gCurFile->cursor.y, gCurFile->row[gCurFile->cursor.y].data,
                      gCurFile->row[gCurFile->cursor.y].size);

      edit->added_range.start_x = gCurFile->row[gCurFile->cursor.y].size;
      edit->added_range.start_y = gCurFile->cursor.y;
      edit->added_range.end_x   = gCurFile->row[gCurFile->cursor.y + 1].size;
    ...
    case CHAR_INPUT:
    {
      ...
      {
        if (gCurFile->bracket_autocomplete &&
            gCurFile->row[gCurFile->cursor.y].data[gCurFile->cursor.x] == c)
        {
          gCurFile->bracket_autocomplete--;
          x_offset = -1;
          gCurFile->cursor.x++;
      ...
      {
        if (gCurFile->row[gCurFile->cursor.y].data[gCurFile->cursor.x] != c)
        {
          editorInsertChar(c);
          editorInsertChar(c);
          x_offset = 1;
